        ...
        for (let j = 0; j < users.userswithchat.length; j++) {
            for (let i = 0; i < users.allUsers.length; i++) {
                if (
                    ...
                    (users.allUsers[i].user ===
                        users.userswithchat[j].chatsender ||
                        users.allUsers[i].user ===
                            users.userswithchat[j].chatrecipient)
                ) {
...
const getNotifications = (users) => {
    ...
    for (const member of userRg) {
        for (const user of users) {
            if (
                member.id == user.notificationsender &&
                user.notificationcount > 0
            ) {
                let notificationDiv =
                    document.createElement("notificationsDiv");
...
function getOneNotification(data) {
    ...
    if (data.notificationrecipient === loggedInUser) {
        let notifSenderDiv = document.getElementById(
...
function addNewRegUserDiv(data) {
    ...
    if (!chatlessUsersNames.includes(data.GetLastUserFromDatabase.user)) {
        ...
        if (
            chatlessUsersNames.indexOf(data.GetLastUserFromDatabase.user) === 0
        ) {
            let userAfterNewUser = chatlessUsersNames[1];
            const userAfterNewUserDiv =
                document.getElementById(userAfterNewUser);
            ...
            let userAfterNewUser =
                chatlessUsersNames[chatlessUsersNames.length - 2];
            const userAfterNewUserDiv =
                document.getElementById(userAfterNewUser);
            ...
            let userAfterNewUser =
                chatlessUsersNames[
                    chatlessUsersNames.indexOf(
                        data.GetLastUserFromDatabase.user
                    ) - 1
                ];
            console.log(
                "INDEX (2) OF NEW USER**********************",
...
*/
            ...
            if (
                chatModal.style.display === "block" &&
                data.livenotification.notificationsender ==
